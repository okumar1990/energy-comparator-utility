...
public class EndpointTest {
    ...
    @Test
    public void givenSmartMeterIdShouldReturnWeeklyElectricityUsage() throws JsonProcessingException {
    	String smartMeterId = "bob";
    	MeterReadingService meterReadingService = new MeterReadingService(new HashMap<>());
        populateMeterReadingsForMeter(smartMeterId);
        List<PricePlan> pricePlans = Arrays.asList(new PricePlan(smartMeterId, null, BigDecimal.TEN, null));
        PricePlanService tariffService = new PricePlanService(pricePlans, meterReadingService);
        ResponseEntity<String> response = restTemplate.getForEntity("/calculateCost/weeklyUsageCost/" + smartMeterId, String.class);
